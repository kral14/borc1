...
import database
from ui_alis_qaime_form import Ui_AlisQaimeForm
from alis_qaime_config import MASTER_COLUMN_LIST
...
class SearchableComboBox(QComboBox):
    def __init__(self, parent=None):
        ...
        self.setEditable(True); self.setInsertPolicy(QComboBox.InsertPolicy.NoInsert)
        self.completer().setCompletionMode(QCompleter.CompletionMode.PopupCompletion)
        self.completer().setFilterMode(Qt.MatchFlag.MatchContains)
        self.placeholder_text = "Məhsul axtarın və ya seçin..."; self.products = []

    def wheelEvent(self, e): e.ignore()

    def focusInEvent(self, e):
        if self.currentText() == self.placeholder_text: self.lineEdit().clear()
        super().focusInEvent(e)

    def focusOutEvent(self, e):
        if self.currentText().strip() == "" and self.currentIndex() <= 0: self.setCurrentIndex(0)
        super().focusOutEvent(e)

    def populate_items(self, products):
        self.products = products; self.blockSignals(True)
        current_data = self.currentData(); self.clear()
        self.addItem(self.placeholder_text, None)
        if self.products:
            for p in self.products: self.addItem(f"{p['name']} (Qalıq: {p['stock']})", p['id'])
...
class ProductSelectorWidget(QWidget):
    ...
    def __init__(self, products, parent=None):
        ...
        self.combo = SearchableComboBox(self); self.combo.populate_items(products)
        self.button = QPushButton(self); icon = self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView)
        self.button.setIcon(icon); self.button.setFixedSize(self.combo.sizeHint().height(), self.combo.sizeHint().height())
        self.button.setToolTip("Mal siyahısından seç (F4)"); layout.addWidget(self.combo); layout.addWidget(self.button)
    ...
    def on_selection_change(self, index):
        ...
        if product_id is not None: self.product_selected_signal.emit()

    def set_current_product(self, product_id):
        if product_id is not None:
            index = self.combo.findData(product_id)
            if index != -1: self.combo.setCurrentIndex(index)
...
class AlisQaimeFormWidget(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.ui = Ui_AlisQaimeForm(); self.ui.setupUi(self)
        self.products = []; self.block_signals = False; self.current_invoice_id = None
        self.original_items = []; self.column_map = {}; self.active_picker_row = -1
        self.setup_toolbar_and_shortcuts(); self.setup_table()
        ...
        self.ui.table_items.installEventFilter(self); self.load_suppliers_to_combobox()
        self.load_products()

    def setup_toolbar_and_shortcuts(self):
        style = self.style(); self.ui.action_add_row.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogNewFolder)); self.ui.action_add_row.setText("Əlavə Et")
        self.ui.action_add_row.setToolTip("Yeni boş sətir əlavə edir (Insert)"); self.ui.action_delete_row.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon)); self.ui.action_delete_row.setText("Sil")
        ...
        copy_icon = style.standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView); self.action_copy_row = QAction(copy_icon, "Kopyala", self)
    ...
    def open_column_settings(self):
        dialog = ColumnSettingsDialog("AlisQaimeTable", MASTER_COLUMN_LIST, self)
        if dialog.exec():
            all_rows_data = []
    ...
    def eventFilter(self, source, event):
        if source is self.ui.table_items and event.type() == QEvent.Type.KeyPress:
            ...
            if event.key() == Qt.Key.Key_F4:
                ...
                return True
        return super(AlisQaimeFormWidget, self).eventFilter(source, event)
    ...
    def add_row(self, item_data=None, is_enabled=True):
        ...
        if item_data:
            ...
            if product_id_val is None or str(product_id_val).strip() == '': product_id = 0
            else: product_id = int(product_id_val)
    ...
    def populate_row_with_product_data(self, row, product_dict):
        self.block_signals = True
        key_map = {'id': 'product_id', 'purchase_price': 'unit_price', 'barcode': 'barcode', 'article': 'article', 'category': 'category'}
        
        # Məhsul adını ComboBox-a yazdırırıq
        product_name_col_idx = self.get_col_index('product_name')
        if product_name_col_idx is not None:
            widget = self.ui.table_items.cellWidget(row, product_name_col_idx)
            if widget:
                widget.combo.lineEdit().setText(product_dict.get('name', ''))
    ...
    def save_invoice(self):
        ...
        if not supplier_id: QMessageBox.warning(self, "Xəta", "Zəhmət olmasa, satıcı seçin."); return
        items_to_save, total_amount = [], 0.0
        for row in range(self.ui.table_items.rowCount()):
            try:
        ...
        func = database.update_purchase_invoice if self.current_invoice_id else database.add_purchase_invoice
        args = [supplier_id, invoice_number, invoice_date, due_date, notes, total_amount, items_to_save]
    ...
    def _fill_form_data(self, invoice_id, enabled):
        details = database.get_purchase_invoice_details(invoice_id)
        if not details or not details.get('invoice'): QMessageBox.critical(self, "Xəta", "Qaimə məlumatları tapılmadı."); return False
        self.original_items = details.get('items', [])
        inv = details['invoice']
        self.ui.edit_invoice_no.setText(inv['invoice_number']); self.ui.date_edit.setDate(inv['invoice_date'])
        if inv.get('due_date'): self.ui.date_due.setDate(inv['due_date'])
        self.ui.edit_notes.setText(inv.get('notes', ''))
        if inv['supplier_id']:
            index = self.ui.combo_supplier.findData(inv['supplier_id'])
        ...
        self.update_totals(); return True

    def set_form_enabled(self, enabled):
        self.ui.combo_supplier.setEnabled(enabled); self.ui.edit_invoice_no.setReadOnly(not enabled)
    ...
    def set_add_mode(self, invoice_number=""):
        ...
        self.load_products(); self.ui.combo_supplier.setCurrentIndex(0)
    ...
    def load_suppliers_to_combobox(self):
        self.ui.combo_supplier.clear(); self.ui.combo_supplier.addItem("Satıcı seçin...", None)
        suppliers = database.get_all_suppliers()
        if suppliers:
            for supplier in suppliers: self.ui.combo_supplier.addItem(supplier['name'], supplier['id'])
...
class AlisQaimeManagerWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.stacked_widget = QStackedWidget(); self.list_widget = AlisQaimeListWidget()
        self.form_widget = AlisQaimeFormWidget(); self.stacked_widget.addWidget(self.list_widget)
    ...
    def show_add_form(self):
        next_invoice_num = database.get_next_purchase_invoice_number()
